...
export default function JustePrix() {
    ...
    const [priceToFind, setPriceToFind] = useState(Math.round(Math.random() * 100));
    const [isDone, setIsDone] = useState(false);
    const [formRestart, setFormRestart] = useState(null);
    const inputPrice = useRef(null);
    const btnSubmit = useRef(null);
    const gifts = ["cadeau 1", "cadeau 11", "cadeau 21", "cadeau 31", "cadeau 41",]
    const [randomGift, setRandomGift] = useState(gifts[Math.ceil(Math.random() * gifts.length - 1)])
            ...
            if (error >= 7) {
                ...
                setFormRestart(
                    <form onSubmit={(e) => handleRestart(e)}>
                        <button type="submit" className="button is-secondary">Recommencer</button>
                    </form>
                );
            }
            return false;
    ...
    return (
        <>
            ...
            <form onSubmit={(e) => handleSubmit(e)} className="has-text-centered">
                ...
                <input type="submit" ref={btnSubmit} value="Proposer" className="button is-primary mt-3" />
            </form>
            {formRestart}
        </>
    )
